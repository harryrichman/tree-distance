...
\begin{rmk*}
	% \farbod{perhaps drop this remark, or change and move it to introduction?}
...
The idea to consider an $\boldm$-like vector as a ``curvature'' also appears in \cite{steinerberger}, which motivates the idea by proving graph-theoretic analogues of some curvature-related theorems in differential geometry. 
...
% \farbod{give reference to \href{https://ems.press/journals/aihpd/articles/16097}{Amini} \cite{amini} and \href{https://link.springer.com/article/10.1007/s00220-023-04879-3}{Brown} \cite{brown} for compuational aspects, using determinants, of (second) Symanzik. 
% Also: I believe Chenxi at some point thought one of Brown's identities might be related to our work...}
...
% \farbod{do we need ``following terminology in...''. kassel-kenyon-wu is used only for this sentence. if there is anything else about that paper that can be added, let's keep this and say more in the related work section}.
...
% or Chaiken~\cite{chaiken} or Bapat~\cite[Theorem 4.7]{bapat}\farbod{is it okay we only keep Tutte reference here?}.
\end{proof}
...
\begin{figure}[h]
	...
	Each forest component is labelled with its oudegree.}
	\label{fig:outdeg}
\end{figure}
...
% In~\cite{devriendt-thesis} Karel Devrient
% \end{rmk}
...
% \farbod{this could be clarified a bit}
...
% \farbod{I was not happy with the subsection titles in this section, and my tweak hasn't helped...} 
...
% \begin{eg}
% 	\farbod{should we keep this, drop it, or move it?}
% Suppose $G$ is the tree with unit edge lengths shown below.
% \[
% \begin{tikzpicture}[scale=0.8]
% 	\coordinate (1) at (0,0);
...
\]
The equilibrium vector (which satisfies $D[S] \boldm = \lambda \mathbf{1}$) in this example is 
\[
	\boldm = \begin{pmatrix} 
	ab + ac & 
	ab + bc &
...
\begin{multline}
	...
	= \frac{1}{2} \Big( (a + b + c + d + e)  \\
	 - \frac{(1^2(abcd + abce + acde + bcde) + 2^2 (abde))}{(abd + abe + acd + ace + ade + bcd + bce + bde)} \Big).
\end{multline}
\end{eg}
