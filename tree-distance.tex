...
In \cite{bapat-sivasubramanian}, Bapat and Sivasubramanian showed the following identity for the sum of cofactors of $D[S]$.
\begin{equation}\label{eq:cof-trees}
	\cof D[S] = (-2)^{|S| - 1} \sum_{T \in \trees(G;S)} w(T) .
\end{equation}
Using \eqref{eq:cof-trees}, an immediate corollary to Theorem~\ref{thm:w-main} is the following identity.
...
\]
It follows that the optimal objective value is
\[
...
\[
	...
	\end{cases}
\]
Summing the terms $(2 - \deg(u))$ against the above over all vertices $u$, we have the following cases:
...
\begin{tikzpicture}[scale=0.6]
	...
	\node[right] at (3) {$s_0$};
\end{tikzpicture}
\caption{Edge $e \in T(s_0)$ with $\delta(e; v, s_0 ) = 1$ (left) and $\delta(e; v, s_0) = 0$ (right).}
\end{figure}
...
\end{multline}
Here we let $\delta(e; v, F(*)) = \delta(e; v, x)$ for any $x \in F(*)$, i.e.
\[
...
\[
	\bigcup_{\substack{s \in S \\ \delta(e;v,s) = 1}} \{ u \in T(s)\} = \begin{cases}
	\displaystyle
	\{ u \in (G \setminus e)^{\overline v} \} &\text{if } e \not\in T, \\[2em]
	\displaystyle
	\left\{ u \in (G \setminus e)^{\overline v} \right\} \cup (T\setminus e)(*)  &\text{if } e \in T(s_0),\, \delta(e;v,s_0) = 0 \\[2em]
	\displaystyle
	\left\{ u \in (G \setminus e)^{\overline v} \right\} \setminus (T\setminus e)(*)  &\text{if } e \in T(s_0),\, \delta(e;v,s_0) = 1 . 
	\end{cases}
\]
alternative:
\[
