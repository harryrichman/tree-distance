...
\end{thm}
\begin{proof}
For $e\in E$ and $v,w\in V$, let $\delta(e;v,w)$ denote the function defined in \note{cite earlier}
%\[
%  \delta(e;v,w) = \begin{cases}
%  1 &\text{if $e$ separates  $v$ from $w$}, \\
%  0 &\text{otherwise}.
...
\begin{align}
  ...
  &= \sum_{e\in E} \alpha_e \sum_{T\in \trees } w(T) \left( \sum_{s \in S} (2 - \deg^o(T, s)) \delta(e; v,s) \right) \\
...
\displaystyle
\bigcup_{s \in S \cap (G\setminus e)^{\overline v}} T(s) &\text{if } e\not\in T, \\[2em]
\displaystyle
\left( \bigcup_{s \in S \cap (G\setminus e)^{\overline v}} T(s) \right) \cup (T\setminus e)_*  &\text{if } e \in T_{s'},\, \delta(e;v,s') = 0 \\[2em]
\displaystyle
\left( \bigcup_{s \in S \cap (G\setminus e)^{\overline v}} T(s) \right) \setminus (T\setminus e)(*)  &\text{if } e \in T_{s'},\, \delta(e;v,s') = 1 . 
\end{cases}
\]
