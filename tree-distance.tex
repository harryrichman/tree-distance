...
\hypersetup{
pdftitle={Minors of tree distance matrices},
}
...
\subjclass[2020]{Primary 05C50; 
Secondary 05C05, 05C12, 05C30, 31C15}
...
\end{equation*}
\end{thm}
[mention Devrient's thesis, Property 3.38]
...
We address this in \cite{richman-shokrieh-wu}, which involves more technical machinery.
...
We have
%Then, Theorem~\ref{thm:w-main} is equivalent to the equation
\begin{equation}
...
Now suppose $|S| = k$ where $2 \leq k < n$, and assume 
    by induction hypothesis 
    that the claim holds for all vertex subsets of size greater than $k$.
    Let $S^+ \subset V$ be a set of $k + 1$ vertices containing $S$.
    The inductive hypothesis states that $D[S^+]$ has $k$ negative eigenvalues and one positive eigenvalue,
    so Cauchy interlacing from $D[S^+]$ implies that $D[S]$ has 
    % at most one positive eigenvalue, and 
    at least $k - 1$ negative eigenvalues. 
Since all diagonal entries of $D[S]$ are zero, $D[S]$ has zero trace. 
Thus the remaining eigenvalue of $D[S]$ must be positive, as claimed.
...
\]
%The Laplacian matrix factors as $\La = B A B^\tr = (B A^{1/2}) (A^{1/2} B^\tr)$
%where $B$ is the incidence matrix and $A$ is the diagonal matrix with entries $\alpha_e^{-1}$.
%Thus
%\begin{align}
%	\boldu^\tr D\boldu 
%	% &= - \frac12 (B^\tr D\boldu)^\tr (B^\tr D\boldu) + 0
%	= -\frac12 \| A^{1/2} B^\tr D \boldu \|^2 \leq 0,
%\end{align}
