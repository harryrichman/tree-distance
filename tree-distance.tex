...
\begin{equation}\label{eq:w-main}
	\det D[S] = (-1)^{|S|-1} 2^{|S|-2} \left( \sum_{E(G)}\alpha_e \sum_{\trees(G;S)} w(\overline{T}) - \sum_{\forests_2(G;S)} (\degout(F,*) - 2)^2 w(F) \right).
\end{equation}
where 
$\trees(G;S)$ is the set of $S$-rooted spanning forests of $G$,
$\forests_2(G;S)$ is the set of $(S,*)$-rooted spanning forests of $G$,
$w(\overline{T})$ and $w(F)$ denote the $\alpha$-weights of the forests $T$ and $F$,
and 
%\begin{equation}
%\label{eq:2-minus-deg}
%k(F,*) 
...
\begin{equation}
\label{eq:det-cof}
	\frac{\det D[S]}{\cof D[S]} = \frac12 \left( \sum_{e \in E} \alpha_e - \frac{\sum_{F \in \forests_2(G; S)} w(F) k(F,*)^2}{\sum_{T \in \trees(G; S)} w(\overline{T})} \right)
\end{equation}
...
Then for any nonempty vertex set $S \subset V$,
\begin{equation}
\det L[\overline S] = \sum_{T \in \trees( G ; S)} w(\overline{T})^{-1}
...
\begin{prop}
For $\boldm$ defined above, $\displaystyle \bone^\tr \boldm = 2 \,\sum_{T \in \trees(G;S)} w(\overline{T})$.
\end{prop}
\begin{proof}
We have
\begin{align}
...
for the constant
\begin{equation}
	\lambda = \sum_{E(G)} \alpha_e \sum_{\trees(G;S)} w(\overline{T}) - \sum_{\forests_2(G;S)} (2 - \degout(F,*) )^2 \, w(F) .
\end{equation}
...
\[
	w(F) = \alpha_e \cdot w(\overline{T}) \qquad\text{if}\qquad F = T \setminus e.
\]
Thus
\begin{align*}
...
\]
\[
	\frac{\det D[S]}{\cof D[S]} = \frac12 \left( \sum_{e \in E} \alpha_e - \frac{\sum_{F \in \forests_2(G; S)} w(F) (\degout(F,*) - 2)^2}{\sum_{T \in \trees(G; S)} w(\overline{T})} \right)
\]
