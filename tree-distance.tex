...
%Compared to this expression for $\delta_{n - k}$, the main result of Graham--Lov\'asz~\cite[Equation (42), p. 81]{graham-lovasz} is longer since it involves sums over spanning forests in $G$ with $(k - 1)$ components, in addition to those with $k$ or $(k + 1)$ components. Their expression is also more complicated in each of the summands. 
...
\begin{align*}
	...
	% &= \sum_{F \in \forests_2} w({F}) (2 - \outdeg(F,*)) \times \\
	% & \Bigg(\! \#\{e \in \partial F(*) \colon \delta(e; v, \pi_{(F \cup e)}(e)) = 1\} - \!\#\{e \in \partial F(*) \colon \delta(e; v, \pi_{(F \cup e)}(e)) = 0\} \Bigg). 
\end{align*}
...
% 	(\star) 
% 	&= \sum_{F \in \forests_2} w({F}) (2 - \outdeg(F, *)) \times \nonumber\\
% 	& \qquad\qquad\qquad   \sum_{e \in \partial F(*)} \Big( \one(\delta(e; v, F(*)) = 0) - \one(\delta(e; v, F(*)) = 1) \Big) \label{eq:penult-new}
% 	% &= \sum_{F \in \forests_2} w({F}) (2 - \outdeg(F,*)) \times \nonumber\\
% 	% & \qquad \Bigg(\! \#\{e \in \partial F(*) \colon \delta(e; v, F(*)) = 1\} - \!\#\{e \in \partial F(*) \colon \delta(e; v, F(*)) = 0\} \Bigg). 
% \end{align}
