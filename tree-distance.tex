...
Given a tree $G$,
let $D$ be the distance matrix of $G$, and $L$ the Laplacian matrix of $G$.
...
\note{check weighted case of above result}
...
If $\lambda_1 \leq \cdots \leq \lambda_n$ are the ordered eigenvalues of $M$ and $\mu_1 \leq \cdots \leq \mu_{n-1}$ are the ordered eigenvalues of $M[\overline{i}]$, then
\[
\lambda_1 \leq \mu_1 \leq \lambda_2 \leq \cdots \leq \mu_{n-1} \leq \lambda_n.
\]
\end{prop}
In other words, the eigenvalues of $M_i$ ``interlace'' the eigenvalues of $M$.
...
\begin{prop}
\label{prop:m-sum}
For the vector $\boldm$ defined above, $\displaystyle \bone^\tr \boldm = 2 \,\sum_{T \in \trees(G;S)} w(\overline{T})$.
\end{prop}
\begin{proof}
By Lemma~\ref{lem:outdeg-sum} we have
\[
\boldm_s = \sum_{T \in \trees(G;S)} (2 - \degout (T,s)) w(\overline{T})
= \sum_{T \in \trees(G;S)} \left( \sum_{v \in T(s)}(2 - \deg(v))\right) w(\overline{T}).
\]
Thus
\begin{align}
	\bone^\tr \boldm = \sum_{s\in S} \boldm_s &= \sum_{s \in S} \left( \sum_{T \in \trees(G;S)} (2 - \degout (T,s)) w(\overline{T}) \right) \\
	&= \sum_{T \in \trees(G;S)} w(\overline{T}) \left( \sum_{s\in S} \sum_{v \in T(s)} 2 - \deg(v) \right) \\
	&= \sum_{T \in \trees} w(\overline{T}) \left( \sum_{v \in V} 2 - \deg(v)\right)
...
\[
	(D[S] \boldm)_v - \sum_{T \in \trees} w(\overline{T}) \sum_{e \in E} \alpha_e = \sum_{T \in \trees} w(\overline{T}) \sum_{e \in E} \alpha_e \sum_{u \in V} (2 - \deg(u)) \left(\delta(e; v, \pi_T(u)) - \delta(e; v, u) \right)
\]
...
for some constant $\lambda$,
and Proposition~\ref{prop:m-sum} states that
$\bone^\tr \boldm = 2 \sum_{T \in \trees(G;S)} w(\overline{T})$.

By Bapat--Sivasubramanian~\cite[Theorem X]{bapat-sivasubramanian},
\[
\bone^\tr \boldm = (-1)^{1 - |S|} 2^{2 - |S|} \cof D[S].
\]

Therefore
\[
\det D[S] = \frac{\cof D[S]}{\bone^\tr D[S]^{-1} \bone}
= \frac{\cof D[S]}{\lambda^{-1} \bone^\tr \boldm}
\]
\[
\frac{\cof D[S]}{\det D[S]} = \bone^\tr D[S]^{-1} \bone
= \frac{1}{\lambda} \bone^\tr \boldm
