...
\det D[S] = (-1)^{|S|-1} 2^{|S|-2} \left( \sum_{E(G)}\alpha_e \sum_{\trees(G;S)} w(T) - \sum_{\forests_2(G;S)} (2 - \degout(F,*))^2 w(F) \right).
\end{equation}
where 
...
\begin{align*}
	(D[S] \boldm)_v &= \sum_{e\in E} \alpha_e \sum_{T\in \trees } w(T) ( 1 - f(v,e,T) )\\
	(D[S] \boldm)_v &= \sum_{T\in \trees } w(T) \left( \sum_{e \not\in T} \alpha_e + \sum_{s_0 \in S} \sum_{e \in T(s_0)} \alpha_e ( 1 - f(T, v,e,s_0) ) \right)\\
\end{align*}
where
$$
