...
\begin{align}
	...
	&=  \frac{\text{\# of $S$-rooted spanning forests of $G$ whose $s_j$-component contains $v$}}{\text{\# of $S$-rooted spanning forests of $G$}}
\end{align}
More equations;
...
\[
	\mathbf{n}(v) = \frac{1}{|\trees(G; S)|} \sum_{T \in \trees(G; S)} \left( \bolddelta(v) - \bolddelta(\pi_T(v)) \right).
\]
...
\begin{align}
	\boldm (G;S)
	&= \sum_{v \in V}(2 - \deg v) {\bolddelta}(v) 
	- \sum_{v \in V} (2 - \deg v) (\bolddelta(v) - \bolddelta(\pi_T(v)) \\
	&= \sum_{v \in V}(2 - \deg v) {\bolddelta}(v) 
	- \kappa(G;S) \sum_{v \in V \setminus S} (2 - \deg v) \mathbf{n}(G;S,v) \\
	&= \boldm(G;V) - \kappa(G;S) \sum_{v\in V\setminus S} (2 - \deg v) \boldn(G;S,v).
\end{align}
\note{TODO: elaborate on this equation}
From Proposition~\ref{prop:m-distance-warmup} we know that $D \boldm(G;V)$ is constant on $V$,
and from Proposition~\ref{prop:n-vector} we know that $D \boldn(G;S,v)$ is constant on $S$ for all $v \in V \setminus S$.
