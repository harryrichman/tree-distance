...
\begin{equation}
\label{eq:12}
	D^{-1} = - \frac12 L + \frac12 \Big( \sum_{e \in E} \alpha_e\Big)^{-1} \boldm\, \boldm^\tr
\end{equation}
...
\end{prop}
\begin{proof}
Multiply \eqref{eq:12} by the all-ones vector $\bone$; since $L \bone = 0$ and $\boldm^\tr \bone = 2$, we obtain $D^{-1} \bone = \Big( \sum_{e \in E} \alpha_e\Big)^{-1} \boldm$.
% Then apply the identity $D \boldm = \left( \sum_{e \in E} \alpha_e \right) \bone$.
Hence $D \boldm = \left( \sum_{e \in E} \alpha_e \right) \bone$.
Then multiply \eqref{eq:12} by $D$ on both sides. 
\end{proof}
...
\begin{align}
	\mathbf{h}^\tr D\mathbf{h} 
	&= - \frac12 \mathbf{h}^\tr DLD \mathbf{h} + 0 \\	
	&= - \frac12 (N^\tr D\mathbf{h})^\tr (N^\tr D\mathbf{h}) + 0
	= -\frac12 \| A^{1/2} N^\tr D \mathbf{h} \|^2 .
\end{align}
\end{proof}
...
%	- \sum_{\substack{e  \in T(s_0) \\ \delta(e;v,s_0) = 0}} 1 \right). \\
	&\qquad\qquad = \sum_{T\in \trees} w(\overline{T}) \sum_{e \in T} \alpha_e ( 2 - \degout(T\setminus e,*)) \Big( \one(\delta(e; v, \pi_T(e)) = 1) - \one(\delta(e; v, \pi_T(e)) = 0) \Big).
\end{align}
