...
\begin{abstract}
We prove a combinatorial formula for 
%the determinant of 
the principal minors of
the distance matrix of a weighted tree.
This generalizes a result of Graham and Pollak.
The identity involves counts of rooted spanning forests of the underlying tree.
\end{abstract}
\maketitle
...
%Weighted version:
If $\{\alpha_e : e\in E\}$ is a collection of positive edge weights,  the $\alpha$-distance matrix $D_\alpha$ takes the distance along edge $e$ to be $\alpha_e$.
Namely, the $(v,w)$-entry of $D_\alpha$ is the sum of the weights $\alpha_e$ along the unique path from $v$ to $w$.
Then
...
\det D_\alpha[S] = (-1)^{|S|-1} 2^{|S|-2} \left( \sum_{E(G)}\alpha_e \sum_{\trees(G;S)} w(T) - \sum_{\forests_2(G;S)} (2 - \degout(F,*))^2 w(F) \right).
\end{equation}
where 
...
we should replace $G$ with this subtree $\conv(S,G)$.
...
Finally, it follows that the optimal objective value is
\[
...
\]
\begin{prop}
If $D[S]$ is any matrix with rows and columns indexed by $S$, then any critical value of the optimization problem \note{cite above} is equal to $\frac{\det D[S]}{\cof D[S]}$
where $\cof D[S]$ denotes the sum of cofactors,
\[
...
\begin{equation}
\label{eq:m-vector}
\mathbf{m}_v =  \sum_{T \in \trees(G;S)} (2 - \degout(T,v)) w(T)
...
%\]
Each $S$-rooted spanning tree $T$ is naturally associated with a surjection $\pi_T: V \to S$, defined by 
\[
	\pi_T(u) = s \qquad\text{if and only if}\qquad u \in T(s).
\]
Using this,
\[
	(D[S] \mathbf{m})_v = \sum_{T \in \trees} w(T) \sum_{e \in E} \alpha_e \left( \sum_{u \in V} (2 - \deg(u)) \delta(e; v,\pi_T(u)) \right)
\]

Regarding the vertex sets that appear in the last summation, we have
\[
	\bigcup_{\substack{s \in S \\ \delta(e;v,s) = 1}} \{ w \in T(s)\} = \begin{cases}
	\displaystyle
	\{ w \in (G \setminus e)^{\overline v} \} &\text{if } e \not\in T, \\[2em]
	\displaystyle
	\left\{ w \in (G \setminus e)^{\overline v} \right\} \cup (T\setminus e)(*)  &\text{if } e \in T(s_0),\, \delta(e;v,s_0) = 0 \\[2em]
	\displaystyle
	\left\{ w \in (G \setminus e)^{\overline v} \right\} \setminus (T\setminus e)(*)  &\text{if } e \in T(s_0),\, \delta(e;v,s_0) = 1 . 
	\end{cases}
\]
alternative:
\[
	\bigcup_{\substack{s \in S \\ \delta(e; v, s) = 1}} \{w \in T(s)\}
	= \begin{cases}
	\displaystyle
	\{w \in V : \delta(e; v, w) = 1 \} &\text{if } e \not\in T, \\[2em]
	\displaystyle
	\left\{ w \in (G \setminus e)^{\overline v} \right\} \cup (T\setminus e)(*)  &\text{if } e \in T,\, s_0(e, T) \in (G \setminus e)^{v} \\[2em]
	\displaystyle
	\left\{ w \in (G \setminus e)^{\overline v} \right\} \setminus (T\setminus e)(*)  &\text{if } e \in T,\, s_0(e,T) \in (G \setminus e)^{\overline v} . 
	\end{cases}
\]
Here 
$T\setminus e$ is a $(S,*)$-rooted forest, i.e. $T \setminus e \in \forests_2(G;S)$, and
$\degout(T\setminus e, *)$ refers to the out-degree of the floating component of $T \setminus e$.
These cases correspond to 
\begin{align*}
	\sum_{s \in S \cap (G\setminus e)^{\overline v}}(2 - \degout(T,s)) = \begin{cases}
	1 &\text{if }e \not \in T, \\
	1 - (2 - \degout(T \setminus e, *)) &\text{if } s_0(e,T) \in (G\setminus e)^{\overline v} , \\
	1 + (2 - \degout(T \setminus e, *)) &\text{if } s_0(e,T) \in (G\setminus e)^v
	\end{cases}
\end{align*}
...
\begin{align*}
	(D[S] \mathbf{m})_v &= \sum_{e\in E} \alpha_e \sum_{T\in \trees } w(T) ( 1 - f(v,e,T) )\\
	(D[S] \mathbf{m})_v &= \sum_{T\in \trees } w(T) \left( \sum_{e \not\in T} \alpha_e + \sum_{s_0 \in S} \sum_{e \in T(s_0)} \alpha_e ( 1 - f(T, v,e,s_0) ) \right)\\
\end{align*}
where
$$
f(T, v,e,s_0) = \begin{cases}
0 &\text{if } e\not\in T\\
2 - \deg^o(T\setminus e,*) &\text{if }  
% e \in T(s_0)\text{ for some } s_0 \in S \text{ with } 
\delta(e;v,s_0) = 0 \\
-(2-\deg^o(T\setminus e,*)) &\text{if }  e \in T(s_0)\text{ for some } s_0 \in S \text{ with } \delta(e;v,s_0) = 1 \\
\end{cases}
$$
Thus
\begin{align}\label{eq:1}
	(D[S] \mathbf{m})(v) - \sum_{T \in \trees} w(T) \sum_{e \in E} \alpha_e 
	&= - \sum_{T\in \trees} w(T) \sum_{s_0 \in S} \sum_{\substack{e \in T(s_0) \\ \delta(e;v,s_0) = 1}} \alpha_e ( 2 - \deg^o(T\setminus e,*)) \\
	&\qquad + \sum_{T \in \trees} w(T) \sum_{s_0 \in S} \sum_{\substack{e  \in T(s_0) \\ \delta(e;v,s_0) = 0}} \alpha_e (2 - \deg^o(T\setminus e,*) )
\end{align}
...
Thus
\begin{align*}
	\eqref{eq:1} &= \sum_{F \in \forests_2} w(F) (2 - \deg^o(F,*)) \sum_{T\in \trees} \sum_{s_0 \in S} \sum_{\substack{e \in T(s_0) \\ \delta(e; v,s_0) = 1}} \one(F = T \setminus e)  \\
	&\qquad - \sum_{F \in \forests_2} w(F) (2 - \deg^o(F,*)) \sum_{T\in \trees} \sum_{s_0 \in S} \sum_{\substack{e \in T(s_0) \\ \delta(e; v,s_0) = 0}} \one(F = T \setminus e) 
\end{align*}
\begin{align*}
	\eqref{eq:1} &= \sum_{F \in \forests_2} w(F) (2 - \deg^o(F,*)) \cdot \#\{T \in \trees : F = T \setminus e \text{ for some } e \in T(s_0),\, \delta(e;v,s_0) = 1 \} \\
	&\qquad - \sum_{F \in \forests_2} w(F) (2 - \deg^o(F,*)) \cdot \#\{T \in \trees : F = T \setminus e \text{ for some } e \in T(s_0),\, \delta(e; v,s_0) = 0 \}
\end{align*}
...
\begin{align*}
	\eqref{eq:1} &= \sum_{F \in \forests_2} w(F) (2 - \deg^o(F,*))  \sum_{e \in \partial F(*)} \one( \delta(e; v, F(*)) = 0)  \\
	&\qquad - \sum_{F \in \forests_2} w(F) (2 - \deg^o(F,*)) \sum_{e\in \partial F(*)} \one(\delta(e; v, F(*)) = 1) 
\end{align*}
Here we let $\delta(e; v, F(*))$ denote
\[
...
Thus
\begin{align*}
	\eqref{eq:1} &= \sum_{F \in \forests_2} w(F) (2 - \degout(F,*))  (\degout(F,*) - 1)  \\
...
\begin{tikzpicture}[scale=0.6]
	...
	\node[right] at (3) {$s_0$};
\end{tikzpicture}
\caption{Graph with $\delta(e; v, s_0 ) = 0$ (left) and $\delta(e; v, s_0) = 1$ (right).}
\end{figure}
...
There is an associated ``addition'' map 
\[
