...
% \todo{add mention of Graham-Lovasz result}
...
\end{mainthm}
Compared to this expression for $\delta_{n - k}$,
the main result of Graham--Lov\'asz~\cite[Equation (42), p. 81]{graham-lovasz}
is longer since it involves sums over spanning forests in $G$ with $(k - 1)$ components, in addition to those with $k$ or $(k + 1)$ components.
Their expression is also more complicated in each of the summands.
Despite selling their result as a ``surprisingly simple'' coefficient expression, it requires three lines to fit on the page, which perhaps explains why the authors were hesitant to state the result earlier in their paper.

%requiring three lines to fit on the page,
%which perhaps explains why the authors were hesitant to state the result earlier in their paper despite selling their coefficient expression as ``surprisingly simple''.
...
\end{equation}
%if $S = V \setminus \{v\}$ contains $n - 1$ vertices, then
%\[
%    \kappa(G; S) = \deg(v), \qquad
%    \kappa_2(G; S) = 1, \quad\text{and}\quad
%    \degout(F, *) = \deg(v)
%\]
%for the unique forest $F \in \forests_2(G; S)$.
%Summing over all vertex subsets with $|S| = n - 1$, we obtain
%by \eqref{eq:sum-of-minors} and \eqref{eq:main} that
%\begin{equation}
%    \delta_1(G) = (-1)^{n - 1} 2^{n - 3} \left( (n - 1) \sum_{v \in V} \deg(v) - \sum_{v \in V} (\deg(v) - 2)^2 \right),
%\end{equation}
%which further simplifies to $
%	\displaystyle \delta_1(G) = (-1)^{n - 1} 2^{n - 3} \left( 2(n - 1)^2 - \sum_{v \in V} (\deg(v) - 2)^2 \right)
%$.
Compare to \eqref{eq:delta-1-graham-lovasz}, which has a different combinatorial flavor.
% The same phenomenon happens for higher $k$ as well.
...
% So, in symbols, $w(H) = w\left(E(H)\right)$.
Let 
\[
...
For an $S$-rooted spanning forest $F$ of $G$, and $s \in S$, define the {\em outdegree} $\outdeg(F,s)$ as the number of edges which join $F(s)$ (the $s$-component of $F$) to a different component; i.e. outdegree of $F(s)$ in the tree $G$.
%\begin{equation}
...
\end{prop}
% \todo{statement of the proposition is shorter, as well as the proof. double check. actually there is something fishy about definitions and D[S] here}
\begin{proof}
...
For simplicity, we state the result for unit edge length trees, but it is clear that a similar result holds for arbitrary edge lengths.
...
% \todo{mathscinet spells O. Amini and Omid Amini as is, we shouldn't have worried about the first two references. but I will keep the edits as is. We don't have reference to our AIHP paper actually, should we? I changed the year for RSW to 2026, should we do 2025? :)}
